package com.zzzzyj.smartpai.consumer;

import com.zzzzyj.smartpai.config.KafkaConfig;
import com.zzzzyj.smartpai.model.FileProcessingTask;
import com.zzzzyj.smartpai.model.FileUpload;
import com.zzzzyj.smartpai.repository.FileUploadRepository;
import com.zzzzyj.smartpai.service.ParseService;
...
public class FileProcessingConsumer {
    ...
    public void processTask(FileProcessingTask task) {
        ...
        try {
            ...
            log.info("步骤1-下载文件耗时: {}ms, fileMd5: {}", System.currentTimeMillis() - step1Start, task.getFileMd5());

            // 解析文件
            long step2Start = System.currentTimeMillis();
            parseService.parseAndSave(task.getFileMd5(), fileStream, 
                    task.getUserId(), task.getOrgTag(), task.isPublic());
            log.info("步骤2-文件解析耗时: {}ms, fileMd5: {}", System.currentTimeMillis() - step2Start, task.getFileMd5());
            ...
            log.info("步骤3-向量化处理耗时: {}ms, fileMd5: {}", System.currentTimeMillis() - step3Start, task.getFileMd5());
            log.info("向量化完成，fileMd5: {}", task.getFileMd5());
            
            log.info("文件处理任务总耗时: {}ms, fileMd5: {}", System.currentTimeMillis() - startTime, task.getFileMd5());
        } catch (Exception e) {
            log.error("Error processing task: {}, 总耗时: {}ms", task, System.currentTimeMillis() - startTime, e);
    ...
    private InputStream downloadFileFromStorage(String filePath) throws ServerException, InsufficientDataException, ErrorResponseException, IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException, InternalException {
        ...
        try {
            ...
            if (filePath.startsWith("http://") || filePath.startsWith("https://")) {
                ...
                // 添加必要的请求头
                connection.setRequestProperty("User-Agent", "SmartRag-FileProcessor/1.0");
